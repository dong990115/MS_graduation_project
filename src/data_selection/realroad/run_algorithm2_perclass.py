...
논문 Definition 4 기반 per-class EIG:
  ...
  - Z' = v13 증강 RAB 데이터 (4,500장, realFOT)
...
OUTPUT_ROOT = os.path.join(MODULE_ROOT, 'Output', 'realroad', RUN_NAME)
K_DIR = os.path.join(OUTPUT_ROOT, f"K_{K}")

EMB_PATH = resolve_emb_path("realroad", "realFOT_noLane_061826_sub4500", MODULE_ROOT, _EMB_ARGS)
...
V13_SRC_DIR = _IMG_LOCAL if os.path.isdir(os.path.join(_IMG_LOCAL, 'LK_CIR_MER_RAB_realFOT')) else r'<DATA_NAS>\MapBuilder\augmented_dataset\realroad_4500'  # 로컬 우선 → NAS 정본
V13_SBEV_DIRS = ["LK_CIR_MER_RAB_realFOT", "drivingAlone_RVL_RAB_realFOT"]
